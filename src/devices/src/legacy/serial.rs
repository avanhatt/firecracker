...
#[cfg(rmc)]
impl EventFd {
    pub fn write(&self, v: u64) -> std::result::Result<(), std::io::Error> {
        unimplemented!()
    }

    pub fn new(flag: i32) -> std::result::Result<EventFd, std::io::Error> {
        unimplemented!()
    }

    pub fn read(&self) -> std::result::Result<u64, std::io::Error> {
        unimplemented!()
    }
}
...
mod tests {
    ...
    impl io::Write for SharedBuffer {
        fn write(&mut self, buf: &[u8]) -> io::Result<usize> {
            if self.loopback {
                self.internal.read_buf.write(buf)
            } else {
                self.internal.write_buf.write(buf)
            }
        }
        fn flush(&mut self) -> io::Result<()> {
            if self.loopback {
                self.internal.read_buf.flush()
            } else {
                self.internal.write_buf.flush()
            }
        }
    }
    impl io::Read for SharedBuffer {
        fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
            ...
            self.internal.read_buf.drain(0..count);
            Ok(count)
        }
    }
    impl AsRawFd for SharedBuffer {
        fn as_raw_fd(&self) -> RawFd {
            self.internal.evfd.as_raw_fd()
        }
    }
    ...
    //     let intr_evt = EventFd::new(libc::EFD_NONBLOCK);
